 ...
 */
export async function alertOnFailure(result) {
    const issues = [];
    ...
    if (issues.length > 0) {
        ...
        const message = issues
            .map((issue) => `• ${issue}`)
            .join("\n");
        await sendTelegramMessage(`🚨 AIPages Crawler Alert${cycleInfo}\n\n${message}\n\n⏰ ${timestamp}`);
    }
}
/**
 ...
 * a degraded state.
 */
export async function alertOnHealthIssue(health) {
    if (health.status === "degraded" || health.store === "disconnected") {
        await sendTelegramMessage(`🚨 AIPages Health Check FAILED\n\nStatus: ${health.status}\nStore: ${health.store}\nTime: ${new Date().toISOString()}`);
    }
}
