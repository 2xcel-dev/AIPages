...
function isPrivateAddress(addr) {
    ...
    if (isIP(addr) === 6) {
        ...
        if (v4map)
            return isPrivateAddress(v4map[1]);
...
function parseIPv6Groups(addr) {
    const halves = addr.split("::");
    if (halves.length > 2)
        return null;
    const parse = (s) => s === "" ? [] : s.split(":").map((g) => parseInt(g || "0", 16));
...
function hostAllowed(hostname) {
    ...
    if (list.length === 0)
        return true; // no allowlist → allow (SSRF still guards IPs)
 ...
 */
export async function guardTargetUrl(rawUrl, opts = {}) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    let addresses;
    try {
        addresses = await lookup(hostname, { all: true, verbatim: true }).then((r) => r.map((a) => a.address));
    }
    catch {
 ...
 */
export async function pinnedRequest(target, opts) {
    ...
    const nodeRes = await new Promise((resolve, reject) => {
        const req = mod.request({
            ...
            family,
            lookup: (_host, _o, cb) => cb(null, target.address, family),
    ...
    const webStream = Readable.toWeb(nodeRes);
 ...
 */
export async function readBodyWithLimit(stream, maxBytes) {
    if (!stream)
...
//# sourceMappingURL=egress-guard.js.map