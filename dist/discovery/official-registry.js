 ...
 */
export const OFFICIAL_REGISTRY_SERVERS_URL = process.env.OFFICIAL_REGISTRY_SERVERS_URL ??
    "https://registry.modelcontextprotocol.io/v0.1/servers";
export const OFFICIAL_SERVERS_FALLBACK_URL = "https://raw.githubusercontent.com/modelcontextprotocol/servers/main/README.md";
/**
 ...
 */
function extractGitHubRepo(url, subfolder) {
 ...
 */
export async function fetchOfficialServersFallback(maxResults = 20, timeoutMs = 8000) {
    ...
    try {
        ...
        const serverRegex = /\[([^\]]+)\]\((?:https:\/\/github\.com\/modelcontextprotocol\/servers\/tree\/main\/)?(src\/[^)]+)\)/g;
        const candidates = [];
        const seen = new Set();
        let match;
 ...
 */
export async function fetchOfficialRegistryCandidates(maxResults = 30, timeoutMs = 8000) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
        ...
        if (!res.ok) {
            console.warn(`[official-registry] Official registry returned status ${res.status}, using reference fallback`);
        ...
        console.warn(`[official-registry] Network error reaching official registry (${err instanceof Error ? err.message : String(err)}), using fallback`);
        return await fetchOfficialServersFallback(maxResults, timeoutMs);
    }
    finally {
        clearTimeout(timer);
