...
export const VALID_CONNECTION_TYPES = ["http", "sse", "stdio", "websocket"];
export const VALID_PRICING_MODELS = ["free", "freemium", "paid"];
export const VALID_HEALTH_STATUSES = ["active", "inactive", "unknown"];
/**
 * Validate a tool record against core directory standards and first-party guardrails.
 */
export function validateToolRecord(raw, options = {}) {
    const errors = [];
    const warnings = [];
    ...
    const isFirstParty = options.isFirstParty ??
        tool.isFirstParty ??
    ...
    if (isFirstParty) {
        if (!FIRST_PARTY_NAMESPACE_REGEX.test(namespace)) {
            errors.push(`First-party AUS tool namespace "${namespace}" must start with "net.2xcel.aus." and contain valid lower-case identifiers (expected pattern: ${FIRST_PARTY_NAMESPACE_REGEX.source})`);
        }
    }
    // 2. Name Validation
    ...
    if (!tool.connectionType || !VALID_CONNECTION_TYPES.includes(tool.connectionType)) {
        errors.push(`Invalid connectionType "${tool.connectionType}". Must be one of: ${VALID_CONNECTION_TYPES.join(", ")}`);
    }
    // 5. Endpoint URL
        ...
        try {
            ...
            if (isFirstParty) {
                ...
                if (parsedUrl.hostname !== CANONICAL_AUS_HOST) {
                    errors.push(`First-party AUS endpoint must reside on canonical host "${CANONICAL_AUS_HOST}", received "${parsedUrl.hostname}"`);
                }
    ...
    if (isFirstParty || tool.capabilities !== undefined) {
        if (!Array.isArray(tool.capabilities) || tool.capabilities.length === 0) {
            ...
            const invalidCaps = tool.capabilities.filter((c) => typeof c !== "string" || !c.trim() || !/^[a-z0-9_-]+$/.test(c.trim()));
            if (invalidCaps.length > 0) {
                errors.push(`Invalid capability tags found: ${JSON.stringify(invalidCaps)}. Tags must be non-empty lowercase alphanumeric strings with hyphens/underscores.`);
            }
        }
    }
    ...
    if (isFirstParty || (tool.pricing?.model === "paid" && tool.connectionType === "http")) {
        if (!tool.authentication || typeof tool.authentication !== "string" || !tool.authentication.trim()) {
            errors.push(`Authentication specification is required for ${isFirstParty ? "first-party AUS" : "paid"} tool`);
        }
            ...
            if (!authLower.includes("x402") && !authLower.includes("x-payment-receipt")) {
                warnings.push(`First-party AUS tool authentication should explicitly describe x402 payment header (received "${tool.authentication}")`);
            }
        }
    }
    ...
    if (tool.healthStatus && !VALID_HEALTH_STATUSES.includes(tool.healthStatus)) {
        errors.push(`Invalid healthStatus "${tool.healthStatus}". Must be one of: ${VALID_HEALTH_STATUSES.join(", ")}`);
    }
    ...
    if (!lastCheckedDate || tool.healthStatus === "unknown") {
        if (healthDerived.reliability !== "unchecked") {
            errors.push(`Anti-Falsification violation: tool without recorded probe timestamp must resolve to reliability "unchecked", got "${healthDerived.reliability}"`);
        }
        if (tool.healthStatus === "active") {
            errors.push("Anti-Falsification violation: tool cannot assert healthStatus 'active' without a recorded lastChecked probe timestamp");
        }
    }
    else {
 ...
 */
export function validateFirstPartyToolRecord(raw, options = {}) {
 ...
 */
export function assertValidFirstPartyTools(tools) {
    const failures = [];
    ...
    if (failures.length > 0) {
        const details = failures
            .map((f) => `  - [${f.namespace}]:\n    ${f.errors.join("\n    ")}`)
            .join("\n");
        throw new Error(`First-party tool validation guardrail failed (${failures.length} invalid tool(s)):\n${details}`);
    }
}
/**
...
export function validateCatalog(tools) {
    ...
    let validCount = 0;
    for (const tool of tools) {
        const isFirstParty = tool.isFirstParty ??
            tool.developer?.isFirstParty ??
