...
export function canonicalString(timestamp, method, pathAndQuery, bodyBytes) {
...
export async function verifyCallerIdentity(opts) {
    ...
    if (!agentId)
        return { ok: false, agentId: "", error: "Missing X-Agent-ID header" };
    ...
    if (!keys || Object.keys(keys).length === 0) {
        ...
        if (!warnedDevPassthrough) {
            ...
            console.warn("[verify] AGENT_KEYS empty — signature verification is DISABLED (dev passthrough). " +
                "Set AGENT_KEYS + VERIFY_REQUIRE_SIGNATURE=true in production.");
        }
        return { ok: true, agentId };
    }
    if (!timestamp)
        return { ok: false, agentId, error: "Missing X-Timestamp header" };
    // Freshness check.
    const ts = Number(timestamp);
    if (!Number.isFinite(ts))
        return { ok: false, agentId, error: "Invalid X-Timestamp" };
    ...
    if (!entry)
        return { ok: false, agentId, error: "Unknown agent — no key registered" };
    ...
    if (entry.address) {
        ...
        if (!rec.ok)
            return { ok: false, agentId, error: rec.error };
    ...
    if (!signature)
        return { ok: false, agentId, error: "Missing X-Signature header" };
