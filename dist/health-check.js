 ...
 */
export async function healthCheck(url, timeoutMs = 8000) {
    if (!url || !url.trim())
        return false;
 ...
 */
export async function healthCheckBatch(items, timeoutMs = 8000) {
    return Promise.all(items.map(async ({ url, namespace }) => {
