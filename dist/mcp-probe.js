 ...
 */
export function parseMcpServersConfig(raw) {
    let doc;
    try {
        doc = JSON.parse(raw);
    }
 ...
 */
export async function probeMcpServer(config, timeoutMs = 30_000) {
...
function probeStdioTools(command, args, env, timeoutMs) {
...
// ── HTTP transport ────────────────────────────────────────────────────────
async function probeHttpTools(url, timeoutMs) {
    const rpc = async (method, params, id) => {
        try {
            ...
            if (id === undefined)
                return {}; // notification — no meaningful response
            if (!res.ok)
                return null;
...
function extractToolsFromList(rawTools) {
    const out = [];
    for (const t of rawTools) {
        ...
        if (!schema)
            continue; // no genuine schema → skip (authoritative only)
        out.push({
            name: t.name,
