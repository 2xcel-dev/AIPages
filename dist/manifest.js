...
function kindFromFilename(path) {
    const base = path.split("/").pop()?.toLowerCase() ?? "";
    if (base === "mcp.json")
        return "mcp";
    if (base === "openapi.json" || base === "openapi.yaml" || base === "openapi.yml")
        return "openapi";
    if (base === "swagger.json" || base === "swagger.yaml" || base === "swagger.yml")
        return "openapi";
    return null;
}
 ...
 */
export async function fetchManifest(repo, knownPath, timeoutMs = 8000) {
    const candidates = [];
    if (knownPath) {
        const k = kindFromFilename(knownPath);
        if (k)
            candidates.push({ kind: k, path: knownPath });
    }
 ...
 */
export function extractTools(repo, manifest, namespacePrefix) {
    const parsed = manifest.kind === "mcp"
...
// ── MCP manifest parser ────────────────────────────────────────────────────
export function parseMcpManifest(raw) {
    let doc;
    try {
        doc = JSON.parse(raw);
    }
    ...
    if (Array.isArray(doc.tools) && doc.tools.length > 0) {
        ...
        for (const t of doc.tools) {
            ...
            out.push({
                ...
                endpointUrl: typeof t.endpointUrl === "string"
                    ? t.endpointUrl
    ...
    if (doc.mcpServers && typeof doc.mcpServers === "object" && !Array.isArray(doc.mcpServers)) {
        ...
        for (const [serverKey, entry] of Object.entries(doc.mcpServers)) {
            ...
            if (hasUrl) {
                ...
                if (tStr.includes("sse"))
                    connType = "sse";
                else if (tStr.includes("ws") || tStr.includes("websocket"))
                    connType = "websocket";
                else
                    connType = "http";
            }
...
function mcpTransport(doc) {
    const raw = doc.transport ?? doc.transportType ?? doc.type ?? "";
    return String(raw).toLowerCase();
}
function mcpToolConnectionType(t, transport) {
    const raw = String(t.transport ?? transport).toLowerCase();
    if (raw.includes("sse"))
        return "sse";
    if (raw.includes("http") || raw.includes("streamable"))
        return "http";
    if (raw.includes("ws") || raw.includes("websocket"))
        return "websocket";
    return "stdio";
}
// ── OpenAPI / Swagger parser ───────────────────────────────────────────────
export function parseOpenApiManifest(raw) {
    let doc;
    try {
        doc = JSON.parse(raw);
    }
...
function schemaFromOpenApi3(op, doc) {
    const content = op.requestBody?.content;
    if (content && typeof content === "object") {
        for (const [mt, media] of Object.entries(content)) {
            if (!String(mt).toLowerCase().startsWith("application/json"))
                continue;
            const norm = normalizeSchema(resolveLocalRef(media?.schema, doc));
            if (norm)
                return norm;
        }
    }
...
function schemaFromSwagger2(op, doc) {
...
function schemaFromParameters(params, doc) {
 ...
 */
function resolveLocalRef(schema, doc) {
 ...
 */
export function normalizeSchema(schemaRaw) {
