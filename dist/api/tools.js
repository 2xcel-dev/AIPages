 ...
 */
export function formatToolRecord(tool) {
 ...
 */
export function matchesSource(tool, sourceQuery) {
    if (!sourceQuery)
        return true;
    ...
    if (q === "official-registry" || q === "official" || q === "registry") {
        ...
        return Boolean(src.startsWith("official-registry") ||
    ...
    if (q === "github") {
        const src = tool.schemaSource?.toLowerCase() ?? "";
        return Boolean(src.startsWith("github") ||
            src.includes("github.com") ||
            tool.namespace.startsWith("github."));
    }
    if (q === "npm") {
        const src = tool.schemaSource?.toLowerCase() ?? "";
        return Boolean(src.startsWith("npm") ||
 ...
 */
export function matchesSearch(tool, searchQuery) {
    if (!searchQuery)
        return true;
 ...
 */
export async function handleGetTools(c, store) {
 ...
 */
export function createToolsApiRouter(getStore) {
